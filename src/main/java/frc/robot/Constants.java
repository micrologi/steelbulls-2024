package frc.robot; //declaração de pacote para organização do código; 
//demonstra que o arquivo java está em uma pasta chamada robot, que está 
//inserido em uma pasta chamada frc.

//bibliotecas importadas para o código.
import com.revrobotics.CANSparkBase.IdleMode;//contém constantes para os 
//modos de inatividade do motor (classe: .IdleMode, importada do pacote: 
//com.revrobotics.CANSparkMax).
import edu.wpi.first.math.geometry.Translation2d;//representa as coordenadas 
//x e y (biodimensional no espaço(classe: .Translation2d, importada do pacote: 
//edu.wpi.first.math.geometry.)).
import edu.wpi.first.math.kinematics.SwerveDriveKinematics;//trata a direção 
//de movimento do swerve (como as rodas devem se movimentar, movimento desejado
//(classe: .SwerveDriveKinematics, importada do pacote: edu.wpi.first.math.kinematics)).
import edu.wpi.first.math.trajectory.TrapezoidProfile;//perfil trapeizdal 
//(aceçleração, desaceleração, velocidade constante e velocidade zero); 
//controle de movimento do robô (classe: .TrapezoidProfile, importada do 
//pacote: edu.wpi.first.math.trajectory).
import edu.wpi.first.math.util.Units;//faz conversões de unidade 
//(metros para pés, radianos para graus e etc.(classe: .Units, 
//importada do pacote: edu.wpi.first.math.util)).
 ...
 */
public final class Constants {//é uma classe (Constants) pública final (que 
  //não pode ser subclasse e, não é alterada após a inicialização, pelo 
  //"final") onde são armazenadas as constantes que serão usadas em 
  //outras partes da programação.
  ...
  */
  ...
  public static final class IntakeConstants {
    public static final int intakeCAN = 30;  //30
    public static final int digitalUpId = 0; 
    public static final int digitalDownId = 1; 
    public static final double inVelocity = 1; // Velocidade de giro do motor - 0.0 a 1.0
  }
